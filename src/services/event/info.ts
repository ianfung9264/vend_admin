...
export const _getBasicEventList = async (): Promise<
  API.DefaultListResponse<API_EVENT.Event>
> => {
  return request("/api/v1/admin/private/event/basic", {
    method: "get",
  });
};
export const _getEventByIdWithApplications = async (eventId: string) => {
  return request(
    `/api/v1/client/public/event/by-id-with-applications?id=${eventId}`,
    {
      method: "get",
    }
  );
...
export const _stopEvent = async ({
  ...
  if (stopResponse && stopResponse.status) {
    try {
      await request(
        `/api/v1/admin/private/event/notify-event-stopped/${event_id}`,
        {
          method: "post",
          data: { reason: reason },
        }
      ...
      console.error(
        "Failed to trigger notification for event stop:",
        notificationError
      );
    }
  }
...
export const _getEventDetailsByEventId = async (
  ...
  const result = await request(
    `/api/v1/client/private/landowner/event/details/admin/${creatorId}/${eventId}`,
    {
      method: "get",
    }
  );
...
export const _approveCancelEvent = async (
  ...
  return request(
    `/api/v1/client/private/landowner/application/refund/admin/${landowner_id}`,
    {
      method: "post",
      data,
    }
