...
const DetailModal: React.FC<DetailModalProps> = ({
  ...
  const fetchEventDataById = async () => {
    ...
    try {
      ...
      const eventDetail = await _getEventDetailsByEventId(
        initData.landownerId,
        initData.eventId
      );
      console.log("eventDetail", eventDetail);

      const totalAmount = eventDetail[0].application_list.reduce(
        (sum: number, app: any) => {
          if (app.status === "Paid") {
      ...
      const paidParticipant = eventDetail[0].application_list.filter(
        (app: any) => app.status === "Paid"
      ).length;
  ...
  const handleRefundModalOk = async () => {
    ...
    const newApplicationIdListForRefundPayload = applicationIdList?.map(
      (app: any) => ({
        id: app.id,
  ...
  return (
    <>
      ...
      >
        ...
        <p>
          <strong>This Landowner's Wallet Balance:</strong> $
          {Number(initData.wallet).toFixed(2)}
        </p>
        <p>
        ...
        <p>
          <strong>Participants who already paid application fee:</strong>{" "}
          {paidParticipant}
        </p>
        <p>
          <strong>Total Amount Received From Participants:</strong> $
          {totalAmount?.toFixed(2)}
        </p>
        <p>
          <strong>Total Amount per Participant:</strong> $
          {(totalAmount / paidParticipant).toFixed(2)}
        </p>
      </Modal>
      <Modal
      ...
      >
        <div style={{ padding: "20px 0" }}>
          ...
          <p>
            if you want to refund 100% for each participant, enter $
            {totalAmount / paidParticipant}
          </p>
          <p>
            if you enter a number larger than {totalAmount / paidParticipant},
            it won't work.
          </p>
          <Input
            prefix="$"
