...
import { Button, Col, Divider, FormInstance, message, Row, Spin, Upload } from "antd";
...
export default function Index() {
	...
	const [termsConditionReload, setTermsConditionReload] = useState(() => termsConditionActionRef.current?.reload);
			...
			<Spin spinning={loading} tip="视频上传中...">
				<ProForm
					...
					submitter={{
						...
						render(props, dom) {
							const editButton = (
								<Button type="primary" onClick={() => setBannerFormReadOnly(false)}>
									Edit
								</Button>
							);
				...
				>
					...
					<ProForm.Group title="Description one">
						<ProFormText colProps={{ span: 12 }} label={"Title"} name={["description1", "title"]} />
						<ProFormText colProps={{ span: 12 }} label={"Subtitle"} name={["description1", "subtitle"]} />
						<ProFormTextArea colProps={{ span: 12 }} label={"body"} name={["description1", "body"]} />
					</ProForm.Group>

					<ProForm.Group title="Description two">
						<ProFormText colProps={{ span: 12 }} label={"Title"} name={["description2", "title"]} />
						<ProFormText colProps={{ span: 12 }} label={"Subtitle"} name={["description2", "subtitle"]} />
						<ProFormTextArea colProps={{ span: 12 }} label={"body"} name={["description2", "body"]} />
					</ProForm.Group>

					<ProForm.Group title="Description three">
						<ProFormText colProps={{ span: 12 }} label={"Title"} name={["description3", "title"]} />
						<ProFormText colProps={{ span: 12 }} label={"Subtitle"} name={["description3", "subtitle"]} />
						<ProFormTextArea colProps={{ span: 12 }} label={"body"} name={["description3", "body"]} />
					</ProForm.Group>
				</ProForm>
			</Spin>
		);
							...
							const createFaq = (
								<BaseModel<API_Setting.CreatePrivacyPolicy>
									...
									modalFormProps={{
										...
										grid: true,
										trigger: <Button icon={<PlusSquareOutlined />} type="text" />,
										submitter: {
											searchConfig: {
												resetText: "Cancel",
												submitText: "Confirm",
									...
									title="New Part"
								>
									<ProFormTextArea colProps={{ span: 18 }} label="Context" name="context" />
								</BaseModel>
							);
