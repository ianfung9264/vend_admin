...
import { ProCard, ProDescriptions, ProForm, ProFormDigit, ProFormText } from "@ant-design/pro-components";
import { useRequest } from "@umijs/max";
import { Button, Col, Divider, FormInstance, message, Row, Tooltip } from "antd";
...
export default function Index() {
	...
	if (initialLoading) {
		return <ProCard title="Fee Setting" className="min-h-[90vh]" loading={true}></ProCard>;
	}
					...
					<Col xs={24} md={12}>
						<ProCard title="Stripe Fee" bordered headerBordered>
							<ProFormDigit
								...
								rules={[{ required: true, message: "Please input Stripe fee percentage!" }]}
							/>
							<ProFormDigit
								...
								min={0}
								fieldProps={{ precision: 2 }}
								rules={[{ required: true, message: "Please input Stripe fixed dollar amount!" }]}
							/>
							<ProFormDigit
								...
								max={100}
								fieldProps={{ precision: 2 }}
								rules={[{ required: true, message: "Please input Stripe billing percentage!" }]}
							/>
						</ProCard>
					</Col>
					...
					<Col xs={24} md={12}>
						<ProCard title="Vend Popups Fee" bordered headerBordered>
							<ProFormDigit
								...
								max={100}
								fieldProps={{ precision: 2 }}
								rules={[{ required: true, message: "Please input VendPopups fee percentage!" }]}
							/>
							<ProFormDigit
								...
								min={0}
								fieldProps={{ precision: 2 }}
								rules={[{ required: true, message: "Please input VendPopups fixed amount!" }]}
							/>
						</ProCard>
					</Col>
				</Row>
