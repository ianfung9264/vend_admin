...
export default function Index() {
  ...
  const expandedRowRender = (record: UnifiedTransaction) => {
    if (
      record.transaction_type === "STALL_PAYMENT" &&
      record.original_application
    ) {
          ...
          >
            <div>
              ...
              <p>
                ...
                <span style={{ marginLeft: "10px" }}>
                  {getStatusTag(stall_payment_status)}
                </span>
              </p>
              <p>
                <strong style={{ display: "block", marginBottom: "4px" }}>
              ...
              {stall_payment_summary ? (
                <div style={{ marginLeft: "10px", fontSize: "0.9em" }}>
                  ...
                  >
                    ...
                    <p style={{ marginLeft: "10px" }}>
                      - Percentage:{" "}
                      {stall_payment_summary.stripe_fee?.percentage || 0}%
                    </p>
                    <p style={{ marginLeft: "10px" }}>
                      - Total Stripe Fee: $
                      {Number(
    ...
    <div>
      <BaseIndex title="All Platform Transactions">
        <Typography.Paragraph type="secondary" style={{ marginBottom: "16px" }}>
          This page provides a comprehensive log of all financial activities
          that occur on the VendPopups platform. Each row represents a distinct
          financial event. You can see three main types of transactions here:
          <br />- <strong>Application Fee:</strong> This is the $2 fee a vendor
          pays when they initially apply to participate in an event.
          <br />- <strong>Stall Payment:</strong> After a vendor's application
          is approved, this is the payment they make for their stall. You can
          click the arrow (or anywhere on the row) for these entries to expand
          them and see a detailed breakdown, including the base ticket price,
          any add-ons, Stripe processing fees, and the commission earned by
          VendPopups.
          <br />- <strong>Refund:</strong> This entry appears when a stall
          payment made by a vendor has been refunded.
          <br />
          The table shows key details like the event name, vendor name,
          transaction amount, status, and date/time for quick reference.
          ...
          props={{
            ...
            request: async (params) => {
              try {
                ...
                console.log(
                  "Debug: appResponse.data details:",
                  appResponse.data
                );
                  ...
                  apps.forEach((app: API_APPLICATION.Application) => {
                    ...
                    if (
                      ...
                      combinedTransactions.push({
                        ...
                        dateTime:
                          app.transaction_summary?.completed_dateTime ||
                          app.updatedAt,
                        original_application: app,
                        application_id: app._id,
                        application_fee_status: app.application_fee_status,
                ...
                if (appResponse && appResponse.data) {
                  if (Array.isArray(appResponse.data)) {
                    processApplicationData(
                      appResponse.data as API_APPLICATION.Application[]
                    );
                  } else if (
                    (
                      appResponse.data as {
                        applications?: API_APPLICATION.Application[];
                      }
                    ).applications &&
                    Array.isArray(
                      (
                ...
                if (refundResponse && Array.isArray(refundResponse)) {
                  refundResponse.forEach((refund: API_APPLICATION.Refund) => {
                    combinedTransactions.push({
                      ...
                      tenant_name: (refund.application as any)
                        ?.tenant_business_name,
                      amount: refund.refunded_amount,
                      status: refund.is_confirm_refunded
                        ? "Confirmed"
                        : "Pending",
