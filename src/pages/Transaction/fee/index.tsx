...
export default function Index() {
  ...
  const CommissionElement = () => {
    const [expandedCommissionKeys, setExpandedCommissionKeys] = useState<
      React.Key[]
    >([]);

    // Calculate total site commission
    const totalSiteCommission = commission.reduce(
      (sum, item: CommissionEntry) => {
        return (
          sum +
          (item.stall_payment_summary?.vendpopups_commission?.total_amount || 0)
        );
      },
      0
    );
            ...
            <div>
              ...
              {stall_payment_summary ? (
                <div style={{ marginLeft: "10px", fontSize: "0.9em" }}>
                  <p>
                    Subtotal: $
                    {Number(stall_payment_summary.sub_total || 0).toFixed(2)}
                  </p>
                  <p>
                    <strong>
                      Final Total: $
                      {Number(stall_payment_summary.final_total || 0).toFixed(
                        2
                      )}
                    </strong>
                  </p>
                  ...
                  {stall_payment_summary.stripe_fee && (
                    ...
                    >
                      ...
                      <p style={{ marginLeft: "10px" }}>
                        - Total Stripe Fee: $
                        {Number(
                          stall_payment_summary.stripe_fee.total_amount || 0
                        ).toFixed(2)}
                      </p>
                    </div>
                  )}
          ...
          props={{
            ...
            columns: [
              ...
              },
              {
                ...
                align: "center",
                render: (text: any) =>
                  text ? `$${Number(text).toFixed(2)}` : "N/A",
              },
              {
            ...
            expandable: {
              ...
              rowExpandable: (record: CommissionEntry) =>
                !!record.stall_payment_summary, // Only expand if there's summary
            ...
            request: async (params) => {
              ...
              let currentCommissionData: CommissionEntry[] =
                remoteData.commission || [];
              ...
              return {
                onClick: () => {
                  if (record.stall_payment_summary) {
                    // Only if expandable
                    const newExpandedKeys = expandedCommissionKeys.includes(
                      record._id
                    )
                      ? expandedCommissionKeys.filter(
                          (key) => key !== record._id
                        )
                      : [...expandedCommissionKeys, record._id];
                    setExpandedCommissionKeys(newExpandedKeys);
  ...
  const TransactionFeeElement = () => {
    ...
    const processApplicationFees = (data: any[]): EventFeeSummary[] => {
      const groupedData: Record<string, EventFeeSummary> = data.reduce(
        (acc, curr) => {
  ...
  return (
    <BaseIndex title="Fee & Commission Overview">
      <Typography.Paragraph type="secondary" style={{ marginBottom: "24px" }}>
        This page provides a comprehensive overview of all fees and commissions
        processed through the VendPopups platform. It's designed to give you,
        the site owner, a clear understanding of revenue generation.
        <br />- The <strong>Dashboard</strong> tab offers a high-level summary:
        total revenue from $2 application fees, total commission earned from
        successful stall payments, and the combined total site earnings.
        <br />- The <strong>Commission</strong> tab lists every stall payment
        that has generated a commission for the platform. You can expand each
        entry to see a detailed financial breakdown, including the original
        stall price, Stripe processing fees, and the net commission earned by
        VendPopups.
        <br />- The <strong>Application Fee</strong> tab shows a breakdown of
        the $2 application fees collected, grouped by each event. This helps
        track how many vendors are applying for specific events.
