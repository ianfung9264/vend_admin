...
export default function Index() {
	...
	const exportToExcel = async () => {
		try {
			...
			const data = response.data.filter((item: any) => item.advanced_status != LandownerAdvancedStatus.WAITING);
			...
			<BaseIndex title="Organization account page">
				...
				<BaseTable<Page_org.mainTable>
					...
					props={{
						...
						request: async () => {
							const dataSource = await _getAllOrg().then(({ data }: { data: Page_org.mainTable[] }) => {
								// data = data.filter(
								//   (item: any) =>
