      ...
      modalFormProps={{
        ...
        onInit: async (values, form) => {
          if (initData?._id) {
            ...
            // Set default value for cooldown_bypass_enabled if not present
            // Convert boolean to string for the select component
            const formData = {
              ...res.data,
              cooldown_bypass_enabled: String(
                res.data.cooldown_bypass_enabled ?? false
              ),
            };
        ...
        onFinish: async (value) => {
          ...
          for (const key in intermediatePayload) {
            if (
              Object.prototype.hasOwnProperty.call(intermediatePayload, key)
            ) {
          ...
          if (finalPayload.hasOwnProperty("be_followed_count")) {
            finalPayload.be_followed_count =
              finalPayload.be_followed_count.toString();
          }

          // Ensure cooldown_bypass_enabled is a boolean
          if (finalPayload.hasOwnProperty("cooldown_bypass_enabled")) {
            finalPayload.cooldown_bypass_enabled =
              finalPayload.cooldown_bypass_enabled === "true";
          }
    ...
    >
      ...
      <ProForm.Group style={{ ...groupStyle }}>
        ...
        <ProFormSelect
          ...
          name="cooldown_bypass_enabled"
          colProps={{ span: 8, offset: 0 }}
          fieldProps={{
            labelInValue: false,
            placeholder: "Select cooldown setting",
          }}
          options={[
            { label: "Normal (3 days)", value: "false" },
            { label: "Bypass Enabled (1 second)", value: "true" },
          ]}
        />
      </ProForm.Group>
      ...
      <ProForm.Group style={{ ...groupStyle }}>
        ...
        >
          {org?.pictures && org.pictures.length > 0 ? (
            org.pictures.map((pic: any, index: number) => (
              <Image key={index} src={pic.url} width={100} />
            ))
          ) : (
            <Text type="secondary">N/A</Text>
          )}
