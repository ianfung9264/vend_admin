...
const WalletBalanceModal: React.FC<WalletBalanceModalProps> = ({ visible, onClose, record }) => {
			...
			Promise.all([
				...
				.then(([withdrawalRes, applicationRes]) => {
					...
					const filtered = combined.filter(
						(tx) =>
							!(
								(tx.transactionType === "application" && tx.status?.toUpperCase() === "WAITING") ||
	...
	const expandedRowRender = (row: any) => {
		if (row.transactionType === "withdrawal") {
			...
			const { stall_payment_summary, tenant_name, ticket_type, add_ons, remark } = appData;
			...
			return (
				<div style={{ padding: "16px" }}>
					<div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "16px" }}>
						<div>
							<p>
								<strong style={{ display: "block", marginBottom: "4px" }}>Transaction ID:</strong>{" "}
							...
							<p>
								...
								<span style={{ marginLeft: "10px" }}>{getStatusTag(row.status)}</span>
							</p>
							<p>
							...
							{stall_payment_summary ? (
								<div style={{ marginLeft: "10px", fontSize: "0.9em" }}>
									...
									<p style={{ marginTop: "8px" }}>
										...
										{stall_payment_summary.stripe_fee_belong === 0 ? "Organizer" : "Platform"}
									</p>
									<p>Total Fee Collected: ${Number(stall_payment_summary.total_fee).toFixed(2)}</p>
									...
									>
										...
										<p style={{ marginLeft: "10px" }}>
											- Billing Percentage: {stall_payment_summary.stripe_fee?.billing_percentage}
											%
										</p>
										<p style={{ marginLeft: "10px" }}>
											- Fixed Dollar: $
									...
									>
										...
										<p style={{ marginLeft: "10px" }}>
											- Percentage: {stall_payment_summary.vendpopups_commission?.percentage}%
										</p>
										<p style={{ marginLeft: "10px" }}>
											- Fixed Amount: $
